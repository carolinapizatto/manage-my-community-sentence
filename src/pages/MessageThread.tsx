...
const MessageThread = () => {
  ...
  const threadMessages = messages.filter(m => m.thread === Number(threadId)).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newMessage: Message = {
      id: Math.max(...messages.map(m => m.id)) + 1,
      subject: threadMessages[0].subject,
      // Keep the original subject without 'Re:'
      content: replyText,
      date: new Date().toLocaleDateString('en-GB', {
        day: 'numeric',
        month: 'long',
  ...
  return <div className="min-h-screen bg-[#f3f2f1]">
      ...
      <main className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="space-y-6">
          {/* Message thread */}
          <div className="space-y-6">
            {threadMessages.map(message => <div key={message.id} className={`flex ${message.sender === "practitioner" ? "justify-start" : "justify-end"}`}>
                <div className={`max-w-[80%] rounded-lg p-4 ${message.sender === "practitioner" ? "bg-[#F1F0FB] text-gray-900 border-l-4 border-[#1d70b8]" : "bg-[#E5DEFF] text-gray-900 border-l-4 border-[#6E59A5]"}`}>
                  <div className="mb-2 flex items-center gap-2">
                    <span className="font-semibold">
                      {message.sender === "practitioner" ? "Your probation practitioner" : "You"}
                    </span>
                    <span className="text-sm text-muted-foreground">{message.date}</span>
                    {message.sender === "practitioner" && !message.read && <Badge variant="destructive" className="ml-auto">New</Badge>}
                  </div>
                  <p className="mb-3">{message.content}</p>
                  
                  {message.hasAttachment && message.attachmentName && <div className="flex items-center gap-2 mt-3 p-2 bg-white rounded">
          ...
          {showEvidenceUpload && <Card className="p-6">
              <form onSubmit={handleSubmitEvidence} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <Textarea id="reason" placeholder="Please explain why you missed your scheduled appointment..." value={reason} onChange={e => setReason(e.target.value)} className="min-h-[150px]" />
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col gap-4">
                    <Input id="evidence" type="file" onChange={handleFileChange} className="flex-1" multiple />
                    
                    {evidenceFiles.length > 0 && <div className="space-y-2">
                        <p className="text-sm font-medium">Uploaded files:</p>
                        <div className="space-y-2">
                          {evidenceFiles.map((file, index) => <div key={index} className="flex items-center justify-between p-2 bg-secondary/50 rounded">
                ...
                <div className="flex justify-end gap-4">
                  <Button type="button" variant="outline" onClick={() => setShowEvidenceUpload(false)}>
                    Cancel
                  </Button>
                  <Button type="submit">
          ...
          {!showReplyForm && !showEvidenceUpload && !isAutomaticReminderThread && <div className="flex gap-3">
          ...
          {isAutomaticReminderThread && <Card className="p-4 bg-gray-100 border-gray-200">
          ...
          {/* Regular reply form */}
          {showReplyForm && <Card className="p-6">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="reply">Your reply</Label>
                  <Textarea id="reply" placeholder="Type your message here..." value={replyText} onChange={e => setReplyText(e.target.value)} className="min-h-[150px]" />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="attachment">Attach supporting evidence (optional)</Label>
                  <div className="flex items-center gap-2">
                    <Input id="attachment" type="file" onChange={e => setAttachment(e.target.files ? e.target.files[0] : null)} className="flex-1" />
                  </div>
                  {attachment && <p className="text-sm text-muted-foreground flex items-center">
